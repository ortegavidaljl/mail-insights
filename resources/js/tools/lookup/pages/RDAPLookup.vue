<template>
	<div :class=" (!report ? 'justify-center' : '') + ' flex flex-col w-full items-center text-white text-center py-16 overflow-y-auto'">
		...
		<div class="sm:text-lg text-base text-start w-11/12 md:w-3/4 lg:w-3/5 xl:w-2/4">
			<input v-model="element" :disabled="isLoading" @keyup.enter="getRDAPData" placeholder="Dominio, IP, ASN, TLD" type="text" class="bg-white shadow-md hover:shadow-xl text-black py-3 px-4 block w-full border-gray-200 shadow-lg rounded-sm focus:z-10 focus:outline-hidden focus:ring-2 ring-blue-500 disabled:bg-slate-50 disabled:text-slate-500 disabled:border-slate-200 disabled:shadow-none"/>
			...
			<table v-if="report && reportSources.length == 0" class="mt-4 min-w-full divide-y-2 divide-gray-200 bg-black/30 rounded-sm">
				<thead>
